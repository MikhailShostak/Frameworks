...
{
public:
    virtual void Parse(const AString &buffer) = 0;
    virtual bool IsComplete() const = 0;
    virtual AString Flush() = 0;
};
...
class CommandParser : public Parser
{
    ...
    bool complete = false;
public:
    void Parse(const AString &buffer) override
    {
        if (complete)
        {
            return;
    ...
    virtual AString HandleCommand(const AString &command)
    {
        return command + "\n";
    }
};
...
public:
    ...
    ASharedPointer<Parser> protocol;
    ...
    void Connect(const AString &host, const AString &port)
    {
