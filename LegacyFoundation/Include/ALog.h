...
struct PUBLIC_API_EXPORT ALogOptions
{
    bool writeToFile = false;
    bool writeToOutput = true;
};

class PUBLIC_API_EXPORT ALogStream
{
...
public:
    ...
    ALogStream &getCategory(const String &category, const char *format, Type &&...values)
    {
        logOptions = &getLogOptionsForCategory(category);
        AMutexLock lock(getMutex());
        writeWithoutBlock(fmt::format(fmt::runtime(format), std::forward<Type>(values)...));
