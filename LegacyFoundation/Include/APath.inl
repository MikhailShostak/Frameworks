...
inline void APath::setPath(const String &path)
{
    ...
    while(lowest < path.size())
    {
        for(SizeType i = lowest; i < path.size(); ++i)
        {
            highest = i;
            if(path[i] == '/' || path[i] == '\\')
            {
        ...
        if(highest == path.size() - 1 && !(path[highest] == '/' || path[highest] == '\\'))
        {
            highest = path.size();
        }
        String name = path.substr(lowest, highest - lowest);
        if(!name.empty() && name != ".")
        {
