...
class PUBLIC_API_EXPORT AAsynchronousPipe : public APipe
{
...
class PUBLIC_API_EXPORT AProcessObserver
{
public:
...
class PUBLIC_API_EXPORT AProcess final
{
...
public:
    ...
    {
        return fmt::format("AProcess({} [{}])", path.toUnixPathString(), Str::Join(arguments, ", "));
    }
...
inline AError AProcess::execute(const ADelegate<void(const AError &error)> &completionDelegate)
{
    ...
    if(executable.isRelative())
    {
        ...
        if (!info.isExists())
        {
            return AError(ExecutableNotFound, fmt::format("Executable at {} was not found", path.toUnixPathString()));
        }
