...
void AWindow::onUpdate(AUpdateEvent &event)
{
    ...
    if (GetGraphicsClock().lastFPSFrame == GetGraphicsClock().currentFrame)
    {
        UITitleChangeEvent e(*this, fmt::format("{} | FPS {} | Frame {} | Objects {}", getTitle(), GetGraphicsClock().currentFPS, GetGraphicsClock().currentFrame, MetaObjectInformation::getInstance().allocatedObjects.size()));
