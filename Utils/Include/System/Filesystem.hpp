...
inline FileTime GetLastWriteTime(const Path& path, bool AllowInexistent = true)
{
    if (AllowInexistent && !IsExists(path))
    {
        return FileTime{};
    }
    else
...
inline FileTime GetLastWriteTime(const Path& path, std::error_code& error_code, bool AllowInexistent = true) noexcept
{
    if (AllowInexistent && !IsExists(path))
    {
        return FileTime{};
    ...
    {
        return std::filesystem::last_write_time(path, error_code);
    }
}
...
inline bool LoadFile(const System::Path &Path, String &Data)
{
...
inline bool SaveFile(const String &Data, const System::Path &Path)
{
