...
struct Profiler
{
    ...
    Profiler()
    {
        ...
        //TODO: Add formater for std::thread::id
        m_Threads.push_back({ fmt::format("Main Thread ({})", std::hash<std::thread::id>{}(id)), id });
    ...
    void RegisterThreads(const Array<std::reference_wrapper<Multithreading::Thread>> &threads, const String &label)
    {
        ...
        for (auto &thread : threads)
        {
            ...
            m_Threads.push_back({ fmt::format("{} #{} ({})", label, i, std::hash<std::thread::id>{}(id)), id });
            ++i;
        }
    }
    ...
    void PrintEvent(std::ofstream &file, ProfilerEvent &e)
    {
        file << "    " << fmt::format(R"(    {{"name": "{}", "ph": "B", "pid": {}, "tid": {}, "ts": {}}})",
            e.Name,
            m_PID,
            std::hash<std::thread::id>{}(e.ThreadID),
...
struct ProfilerScopeRecord
{
    ...
    ProfilerScopeRecord(const fmt::format_string<Arguments...> &format, Arguments &&...arguments)
    {
