...
public:
    ...
    void NewFrame(const Graphics::SwapChain &SwapChain)
    {
        Textures.clear();

        //const auto &SCDesc = g_CurrentImGui->SwapChain.Data->Handle->GetDesc();
        //VERIFY(SCDesc.PreTransform == Diligent::SURFACE_TRANSFORM_IDENTITY, "Unexpected surface pre-transform");
        ...
        //ImGuiImplDiligent::NewFrame(SCDesc.Width, SCDesc.Height, SCDesc.PreTransform);
        /*
...
void BeginRender(void* window)
{
    ...
    g_CurrentImGui->NewFrame(g_CurrentImGui->SwapChain);
