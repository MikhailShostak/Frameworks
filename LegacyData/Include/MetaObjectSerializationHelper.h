...
template<>
inline AJSONValue toJSONValue<Color>(const Color &value)
{
    AJSONObject object;
    object.insert("X", value[0]);
    object.insert("Y", value[1]);
    object.insert("Z", value[2]);
    object.insert("W", value[3]);
    return object;
}

template<>
inline void fromJSONValue<Color>(const AJSONValue &value, Color &output)
{
    if (!value.isObject())
    {
        return;
    ...
    object.read("Red", output[0]);
    object.read("Green", output[1]);
    object.read("Blue", output[2]);
    object.read("Alpha", output[3]);

    object.read("X", output[0]);
    object.read("Y", output[1]);
    object.read("Z", output[2]);
    object.read("W", output[3]);
}
